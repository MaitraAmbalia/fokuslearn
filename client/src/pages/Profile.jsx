  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 transition-colors duration-300">
      ...
      <main className="max-w-6xl mx-auto px-4 py-8">
        ...
        {/* --- Header Section --- */}
        <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-700 p-8 mb-8 flex flex-col md:flex-row items-center md:items-start gap-8 relative group">
          {/* Edit & Logout Buttons */}
          <div className="absolute top-6 right-6 flex items-center gap-2">
            ...
            <button
              onClick={logout}
              className="p-2 text-slate-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-slate-700 rounded-full transition-all"
              title="Logout"
            >
          ...
          <div className="relative shrink-0">
            <div className="w-32 h-32 rounded-full p-1 bg-gradient-to-tr from-blue-500 to-purple-500">
              <div className="w-full h-full rounded-full border-4 border-white dark:border-slate-800 bg-gradient-to-br from-blue-400 to-purple-500 flex items-center justify-center text-white text-4xl font-bold overflow-hidden">
                {displayUser?.avatar ? (
                  <img
            ...
            <div className="absolute -bottom-2 -right-2 bg-slate-900 text-white text-xs font-bold px-3 py-1 rounded-full border-4 border-white dark:border-slate-800">
          ...
          <div className="flex-1 text-center md:text-left space-y-3">
            <div>
              <h1 className="text-3xl font-bold text-slate-900 dark:text-white">{displayUser?.name}</h1>
            </div>

            {/* Bio Section */}
            {displayUser?.bio && (
              <p className="text-slate-600 dark:text-slate-300 text-sm max-w-lg mx-auto md:mx-0 leading-relaxed font-sans">
                {displayUser.bio}
              </p>
            )}
        ...
        <div className="flex gap-8 border-b border-slate-200 dark:border-slate-700 mb-8">
          <button
            ...
            className={`pb-3 text-sm font-semibold transition-all ${activeTab === 'overview' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200'}`}
          >
            Overview
          </button>
          <button
            onClick={() => setActiveTab('certificates')}
            className={`pb-3 text-sm font-semibold transition-all ${activeTab === 'certificates' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200'}`}
          >
            Certificates <span className="ml-1 bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 px-2 py-0.5 rounded-full text-xs">{displayUser?.certificates?.length || 0}</span>
          </button>
        </div>
        ...
        {activeTab === 'overview' ? (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-1 space-y-6">
              <h3 className="font-bold text-lg text-slate-800 dark:text-white">Active Courses</h3>
              {goals.length === 0 ? (
                <div className="text-center p-8 bg-white dark:bg-slate-800 rounded-2xl border border-dashed dark:border-slate-700 text-slate-400">
                  No active courses.
                </div>
              ) : (
                goals.map((goal) => (
                  <div key={goal._id} className="bg-white dark:bg-slate-800 p-4 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm hover:shadow-md transition-shadow">
                    <div className="flex gap-4 mb-3">
                      <img src={goal.thumbnail} alt="" className="w-16 h-16 rounded-lg object-cover bg-slate-100 dark:bg-slate-700" />
                      <div className="flex-1 min-w-0">
                        <h4 className="font-bold text-slate-800 dark:text-white truncate">{goal.title}</h4>
                        <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">{goal.completedVideos} / {goal.totalVideos} Videos</p>
                      </div>
                    </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-in fade-in zoom-in-95 duration-300">
            {displayUser?.certificates && displayUser.certificates.length > 0 ? (
              displayUser.certificates.map((cert, index) => (
                <div key={index} className="bg-white dark:bg-slate-800 p-4 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm flex flex-col gap-4 hover:shadow-md transition-all group">
                  {/* Image Preview */}
                  <div className="relative aspect-[1.41] bg-slate-100 dark:bg-slate-900 rounded-lg overflow-hidden border border-slate-100 dark:border-slate-700">
                  ...
                  {/* Info */}
                  <div>
                    <h3 className="font-bold text-slate-900 dark:text-white line-clamp-1" title={cert.courseTitle}>{cert.courseTitle}</h3>
...
const StatCard = ({ icon: Icon, label, value, color, bg }) => (
  <div className="bg-white dark:bg-slate-800 p-4 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm flex flex-col items-center justify-center text-center gap-2 hover:shadow-md transition-shadow">
    <div className={`p-3 rounded-full ${bg} ${color} dark:bg-opacity-20`}>
